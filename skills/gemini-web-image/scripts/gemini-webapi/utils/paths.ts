...
const APP_DATA_DIR = 'gemini-web-image';
const GEMINI_DATA_DIR = 'data';
...
export function resolveGeminiWebConsentPath(): string {
  return path.join(resolveGeminiWebDataDir(), CONSENT_FILE_NAME);
}
...
export function resolveGeminiWebChromeProfileDir(): string {
  ...
  return wslHome
    ? path.join(wslHome, '.local', 'share', APP_DATA_DIR, PROFILE_DIR_NAME)
    : path.join(resolveUserDataRoot(), APP_DATA_DIR, PROFILE_DIR_NAME);
}
