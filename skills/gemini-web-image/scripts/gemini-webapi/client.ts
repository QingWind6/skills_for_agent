  ...
  async generate_content(
    ...
    return await this._run(async () => {
      ...
      try {
        ...
        logger.debug(`Invalid response: ${responsePreview}`);
        throw new APIError(
          `Failed to generate contents. Invalid response data received. Client will try to re-initialize on next request. Response preview: ${responsePreview}`,
        );
      }
